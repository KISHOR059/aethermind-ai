    ...
    <Card>
      <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
        ...
        <div className="flex shrink-0 gap-1">
          ...
          <Button aria-label={`Delete ${task.title}`} size="icon-sm" variant="ghost" onClick={() => setConfirmDelete(true)}><Trash2 /></Button>
        </div>
      </CardHeader>
      ...
      <ConfirmDialog open={confirmDelete} onOpenChange={setConfirmDelete} title="Delete task?" description={`This will remove “${task.title}” from your task list.`} confirmLabel="Delete" onConfirm={() => deleteTask.mutate(task.id, { onSuccess: () => notify.success("Task deleted"), onError: (error) => notify.error("Unable to delete task", error.message) })} />
    </Card>
  );
}
